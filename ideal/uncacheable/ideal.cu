...
int main( int argc, char *argv[] )
{
    ...
    switch (benchmarkType) {
       ...
       case 1: {// cudaHostAlloc
                  ...
                  cpu_compute(in, out, numBytes);
                  gettimeofday(&tv2, NULL);
                  double elapsedTimeSecondsBefore = diff_s(tv1,tv2)/10;
//////////////////////kernel 1
                  double elapsedTimeSecondsAfter = 0;
                  ...
                  cpu_compute(in, out, numBytes);
                  gettimeofday(&tv2, NULL);
                  elapsedTimeSecondsAfter += diff_s(tv1,tv2);
//2
                  kernel<<<num_of_blocks,num_of_threads_per_block>>>(in,out);
                  HANDLE_ERROR(cudaDeviceSynchronize());
...
//10
                  ...
                  gettimeofday(&tv2, NULL);
                  elapsedTimeSecondsAfter += diff_s(tv1,tv2);
                  elapsedTimeSecondsAfter = elapsedTimeSecondsAfter/10;



                  verify(in,out,numBytes);
                  HANDLE_ERROR( cudaGetLastError());
                  printf("CPU compute cudaHostAlloc Before = %f ms\n",elapsedTimeSecondsBefore*1e3/(float)ITERATIONS);
                  printf("CPU compute cudaHostAlloc After  = %f ms\n",elapsedTimeSecondsAfter*1e3/(float)ITERATIONS);
...
//10
                  ...
                  verify(in,out,numBytes);
                  HANDLE_ERROR( cudaGetLastError());
                  printf("CPU compute cudaMallocManaged Before = %f ms\n",elapsedTimeSecondsBefore*1e3/(float)ITERATIONS);
                  printf("CPU compute cudaMallocManaged After  = %f ms\n",elapsedTimeSecondsAfter*1e3/(float)ITERATIONS);
